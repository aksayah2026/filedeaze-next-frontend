...
export default function ManagerLogin() {
  ...
  const onSubmit = async ({ tenantCode, email, password }: Form) => {
    try {
      ...
      if (user.role === 'MANAGER' || user.role === 'ADMIN') router.push('/manager/dashboard');
      else toast.error('Unauthorized role for this portal');
    } catch (err: unknown) {
    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-900 p-4">
      <div className="w-full max-w-sm">
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-sm text-slate-400 mt-1">Manager Portal</p>
        </div>
        <form onSubmit={handleSubmit(onSubmit)} className="bg-white rounded-2xl p-8 shadow-2xl space-y-4">
          ...
          <Input label="Email" type="email" placeholder="manager@company.com" {...register('email')} error={errors.email?.message} />
          <Input label="Password" type="password" placeholder="••••••••" {...register('password')} error={errors.password?.message} />
          <Button type="submit" loading={isSubmitting} className="w-full mt-2">Sign In</Button>
          <p className="text-center text-xs text-gray-400 mt-2">
            Admin?{' '}
            <a href="/admin/login" className="text-blue-600 hover:underline">Use Admin Portal</a>
          </p>
        </form>
      </div>
    </div>
